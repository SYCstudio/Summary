...
\begin{document}
	...
	\section{其它}
		\subsection{STL}
			...
			\mintinline{c++}{make\_heap}: 根据指定的迭代器区间以及一个可选的比较函数，来创建一个 $heap$。 复杂度 $O(n)$
			
			\mintinline{c++}{push\_heap}: 把指定区间的最后一个元素插入到 $heap$中。复杂度 $O(\log n)$
			
			\mintinline{c++}{pop\_heap}: 弹出 $heap$ 顶元素, 将其放置于区间末尾。复杂度 $O(\log n)$
